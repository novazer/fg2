...
class DeviceService {
  ...
  private async checkDeviceClasses() {
    for(let device_class of minimal_classes) {
  ...
  async connectMqtt() {
    try {
      ...
      mqttclient.messages.subscribe(async (message) => {
        let device_id = message.topic.split('/')[2];
        let topic = message.topic.split('/')[3];
  ...
  private async findUpgradeableDevices() {
    ...
    for(let device_class of classes) {
      const currently_upgrading = await deviceModel.where({
      ...
      const failed = await deviceModel.where({
        pending_firmware: device_class.firmware_id,
        class_id: device_class.class_id,
        current_firmware: {$ne: device_class.firmware_id},
  ...
  public async getDeviceLogs(device_id:string, user_id:string, is_admin:boolean) {
    ...
    if(device) {
      let logs = await deviceLogModel.find({device_id: device_id}).sort({time: 1}).limit(100)
      return logs;
    }
    return [];
  }
  ...
  public async deleteDeviceLogs(device_id:string, user_id:string) {
    let device = await deviceModel.findOne({device_id: device_id, owner_id: user_id}, { device_id: 1 });
    if(device) {
  ...
  public async register(info:RegisterDeviceDto): Promise<any> {
    ...
    try {
      let serialquery = await deviceModel.aggregate(
        [
          {
            $group:
              {
  ...
  private genClaimCode():string {
    const chars = ["A", "B" , "C", "D", "E", "F", "G", "H", "K", "M", "N", "P", "R", "S", "T", "U", "V", "W", "X", "Y", "Z", "2", "3", "4", "5", "6", "7", "8", "9"];
    const len = 6;
    let code = "";
  ...
  public async getClaimCode(device_id:string) {
    ...
    await claimCodeModel.findOneAndUpdate(
      {device_id: device_id},
      {claim_code: code, device_id: device_id},
      {upsert: true}
    )
  ...
  public async claimDevice(claim_code:string, user_id:string) {
    let dev = await claimCodeModel.findOne({claim_code: claim_code});
    if(dev) {
  ...
  public async getDeviceConfig(device_id:string, user_id:string, is_admin:boolean) {
    if(is_admin) {
      let device = await deviceModel.findOne({device_id: device_id}, { configuration: 1 });
      return device.configuration;
    }
    else {
      let device = await deviceModel.findOne({device_id: device_id, owner_id: user_id}, { configuration: 1 });
      return device.configuration;
    }
  }
  ...
  public async testOutputs(device_id: string, outputs:TestDeviceDto) {
    mqttclient.publish('/devices/' + device_id + '/command', JSON.stringify({
      action: "test",
      outputs: {
  ...
  public async updateClass(class_id:string, name: string, description: string, concurrent: number, maxfails: number, firmware_id:string): Promise<DeviceClass> {
    const update = await deviceClassModel.findOneAndUpdate({class_id: class_id}, {
      name: name,
      description: description,
      concurrent: concurrent,
      maxfails: maxfails,
  ...
  public async findOnlineDevices(): Promise<any> {
    ...
    const class_count = await Promise.all(classes.map(async (deviceclass) => {
      return {
        class: deviceclass,
  ...
  public async getFirmwareVersions(): Promise<any> {
    ...
    const class_count = await Promise.all(classes.map(async (deviceclass) => {
      ...
      versions.push({
        ...
        online: await deviceModel.where({lastseen: {$gte: Date.now() - ONLINE_TIMEOUT}, class_id: deviceclass.class_id, current_firmware: {$nin: fwids}}).countDocuments(),
